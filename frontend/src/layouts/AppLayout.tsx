...
export default function AppLayout() {
  ...
  const nav = (
    <nav className="space-y-5 overflow-y-auto pr-1" aria-label="Primary">
  ...
  return (
    <div className="min-h-screen bg-background text-textPrimary">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 flex-col bg-sidebar p-5 text-white lg:flex">
        <div className="mb-8 px-2">
          ...
          <h1 className="mt-3 text-xl font-semibold leading-snug">National Insurance Platform</h1>
          <p className="mt-2 text-xs text-emerald-100/70">Version 1.0</p>
        </div>
        ...
        <div className="mt-4 rounded-2xl border border-white/10 bg-white/10 p-4">
          ...
          <p className="mt-1 text-sm text-emerald-100/80">{user?.roles?.[0] ?? 'Operator'}</p>
        </div>
      </aside>
